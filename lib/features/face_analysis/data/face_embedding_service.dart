...
class FaceEmbeddingService {
  ...
  Future<List<double>> embed({
    ...
    final inputTensor = await OrtValue.fromList(
      Float32List.fromList(
        _toModelTensor(_cropFace(source, face.boundingBox)),
      ),
      [1, 3, 112, 112],
    );
    ...
    try {
      ...
      stopwatch.stop();
      debugPrint(
        'Shabah ONNX inference: ${stopwatch.elapsedMilliseconds} ms',
      );
  ...
  Future<File> _ensureModelFile({
    ...
    try {
      ...
      onStatus?.call('تم تجهيز نموذج الوجه.');
      return file;
    } finally {
  ...
  Future<void> _downloadResponse(
    ...
    try {
      await for (final chunk in response.timeout(
        ...
        onTimeout: (sink) {
          sink.addError(
            TimeoutException('لم تصل بيانات نموذج الوجه لمدة 30 ثانية.'),
          );
          sink.close();
        },
      )) {
        ...
        } else if (received > 0 && lastPercent < 0) {
          lastPercent = 0;
          onStatus?.call('جاري تنزيل نموذج الوجه…');
        }
      }
    } finally {
      await sink.close();
  ...
  img.Image _cropFace(img.Image source, Rect box) {
    ...
    final cropLeft =
        (cx - size / 2).round().clamp(0, source.width - 1);
    final cropTop =
        (cy - size / 2).round().clamp(0, source.height - 1);
    final cropRight =
        (cropLeft + size).clamp(cropLeft + 1, source.width);
    final cropBottom =
        (cropTop + size).clamp(cropTop + 1, source.height);
