...
/// On-device 512-dimensional face embedding using LibreFaceRec / AuraFace-v1.
/// The face is first aligned to the canonical ArcFace 112x112 five-point template.
class FaceEmbeddingService {
  ...
  Future<List<double>> embed({required String imagePath, required Face face}) async {
    final session = await _getSession();
    final source = img.decodeImage(await File(imagePath).readAsBytes());
    if (source == null) throw StateError('تعذر قراءة الصورة.');

    final aligned = _alignment.align(source, face);
    if (aligned == null) throw StateError('تعذر محاذاة نقاط الوجه الخمس.');

    final input = _toModelTensor(aligned);
    ...
    try {
      ...
      if (outputs != null) {
        for (final output in outputs.values) await output.dispose();
      }
    }
  }
