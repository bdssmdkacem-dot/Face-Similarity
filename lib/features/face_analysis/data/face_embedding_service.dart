...
class FaceEmbeddingService {
  ...
  Future<List<double>> embed({required String imagePath, required Face face, void Function(String status)? onStatus}) async {
    ...
    final session = await _getSession(onStatus: onStatus).timeout(const Duration(seconds: 150), onTimeout: () => throw TimeoutException('تهيئة نموذج الوجه تجاوزت المهلة.'));
    ...
    try {
      ...
      final values = (await output.asFlattenedList()).map((value) => (value as num).toDouble()).toList(growable: false);
  ...
  Future<File> _ensureModelFile({void Function(String status)? onStatus}) async {
    ...
    try {
      ...
      await _downloadResponse(response, file, onStatus: onStatus).timeout(const Duration(minutes: 5), onTimeout: () => throw TimeoutException('تنزيل نموذج الوجه تجاوز 5 دقائق.'));
      onStatus?.call('جاري التحقق من نموذج الوجه…');
      final digest = await sha256.bind(file.openRead()).first;
      if (digest.toString() != _expectedSha256) { await file.delete(); throw StateError('فشل التحقق من سلامة نموذج الوجه.'); }
      return file;
    } finally { client.close(force: true); }
  }

  Future<void> _downloadResponse(HttpClientResponse response, File file, {void Function(String status)? onStatus}) async {
    ...
    try {
      await for (final chunk in response.timeout(const Duration(seconds: 30))) {
        ...
        if (total > 0) { final percent = (received * 100 / total).clamp(0, 100).round(); if (percent >= lastPercent + 5) { lastPercent = percent; onStatus?.call('جاري تنزيل نموذج الوجه… $percent%'); } }
      }
    } finally { await sink.close(); }
  }

  img.Image _cropFace(img.Image source, Rect box) {
    final left = box.left.floor().clamp(0, source.width - 1); final top = box.top.floor().clamp(0, source.height - 1);
    final right = box.right.ceil().clamp(left + 1, source.width); final bottom = box.bottom.ceil().clamp(top + 1, source.height);
    final width = right - left, height = bottom - top, size = math.max(width, height);
    final cx = left + width / 2, cy = top + height / 2;
    final cropLeft = (cx - size / 2).round().clamp(0, source.width - 1); final cropTop = (cy - size / 2).round().clamp(0, source.height - 1);
    final cropRight = (cropLeft + size).clamp(cropLeft + 1, source.width); final cropBottom = (cropTop + size).clamp(cropTop + 1, source.height);
  ...
  List<double> _toModelTensor(img.Image image) {
    final values = <double>[];
    for (var channel = 0; channel < 3; channel++) for (var y = 0; y < 112; y++) for (var x = 0; x < 112; x++) { final pixel = image.getPixel(x, y); final value = channel == 0 ? pixel.r : channel == 1 ? pixel.g : pixel.b; values.add((value.toDouble() - 127.5) / 127.5); }
    return values;
  }
